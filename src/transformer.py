...
class GraphTransformer(nn.Module):
    ...
    def forward(self, input_data):
        ...
        src, src_mask, adj_matrix, distances_matrix, edges_att = input_data
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, adj_matrix, distances_matrix, edges_att, mask=None, output_attn=False):
        "Implements Figure 2"
        if mask is not None:
        ...
        x, self.attn, self_attn_mat = attention(query, key, value, adj_matrix, 
                                                p_dist, edges_att,
        ...
        if output_attn:
            return self.linears[-1](x), self.attn, self_attn_mat
