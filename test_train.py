...
def test_model(model, test_X, test_y):
    ...
    print(f"Mean loss of {test_loss/num_data} over {num_data} samples")
    return test_loss
...
train_x = X[:-args.withhold]
train_y = y[:-args.withhold]
test_x = X[-args.withhold:]
test_y = y[-args.withhold:]
...
for i in range(epoch//20):
    train_model(model, data_loader, 20, learning_rate)
    test_model(model, test_x, test_y)

torch.save(model.state_dict(), f"{args.label}_{datetime.datetime.now().strftime('%b%d_%H%M')}.torch")
